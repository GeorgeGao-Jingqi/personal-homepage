...
function validDocument(value: unknown): value is { frontmatter: Record<string, unknown>; body: string } { if (!value || typeof value !== "object") return false; const item = value as { frontmatter?: Record<string, unknown>; body?: unknown }; const data = item.frontmatter; return Boolean(data && typeof data.title === "string" && typeof data.summary === "string" && typeof data.date === "string" && typeof data.updated === "string" && typeof data.publish === "boolean" && statuses.has(String(data.status)) && list(data.tags) && list(data.related) && typeof item.body === "string"); }
function notePath(type: string, slug: string) { return types.has(type) && safeSlug(slug) ? resolve(notesRoot, type, `${slug}.md`) : null; }
function documentFromFile(raw: string) { const parsed = matter(raw); const data = parsed.data as Record<string, unknown>; return { frontmatter: { title: String(data.title ?? ""), status: String(data.status ?? "draft"), publish: data.publish === true, date: String(data.date ?? ""), updated: String(data.updated ?? data.date ?? ""), summary: String(data.summary ?? ""), tags: Array.isArray(data.tags) ? data.tags.map(String) : [], related: Array.isArray(data.related) ? data.related.map(String) : [], ...(typeof data.source === "string" ? { source: data.source } : {}), ...(typeof data.sourceUrl === "string" ? { sourceUrl: data.sourceUrl } : {}) }, body: parsed.content.trim() }; }
...
function localApi() {
  return { name: "local-garden-api", configureServer(server: import("vite").ViteDevServer) {
    server.middlewares.use("/api/notes", async (req, res) => {
      ...
      if (req.method === "PUT") { try { const input = JSON.parse((await body(req)).toString()); if (!validDocument(input)) return send(res, 400, "笔记内容无效"); const frontmatter = { ...input.frontmatter, publish: input.frontmatter.status === "published", tags: list(input.frontmatter.tags), related: list(input.frontmatter.related) }; const raw = matter.stringify(`\n${input.body.trim()}\n`, frontmatter).replace(/^(date|updated): ['"](\d{4}-\d{2}-\d{2})['"]$/gm, "$1: $2"); await writeFile(path, raw, "utf8"); return send(res, 204); } catch { return send(res, 400, "无法保存笔记"); } }
